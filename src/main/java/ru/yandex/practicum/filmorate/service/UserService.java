...
public class UserService {
    ...
    public User deleteById(int id) {
        if (!userStorage.getUsers().containsKey(id))
            throw new NotFoundException("Пользователь не найден, невозможно удалить.");
    ...
    public List<User> addFriendship(int firstId, int secondId) {
        if (!userStorage.getUsers().containsKey(firstId) || !userStorage.getUsers().containsKey(secondId))
            throw new NotFoundException(String.format("Пользователя с id %d или %d не существует", firstId, secondId));
    ...
    public List<User> getFriendsListById(int id) {
        if (!userStorage.getUsers().containsKey(id)) throw new NotFoundException("Пользователь не найден");
